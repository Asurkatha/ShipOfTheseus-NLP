...
Prior work on authorship attribution under adversarial conditions \cite{koppel2011authorship} demonstrates that paraphrasing degrades attribution accuracy, but does not quantify the \emph{rate} or \emph{order} of feature decay across iterations. Work on AI text detection \cite{mitchell2023detectgpt} focuses on binary classification (human vs.\ machine) rather than measuring how iterative rewriting erodes specific linguistic features at different rates. Neural paraphrase typologies \cite{krishna2023paraphrase} characterize the \emph{types} of changes paraphrasers make, but do not track how those changes accumulate across multiple generations.
